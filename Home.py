...
OPENROUTER_API_KEY = "Put_here_your_api_key"
...
def execute_safe_query(query: str) -> List[Dict[str, Any]]:
    ...
    # Convert tuple of tuples to list of dicts if needed
    if isinstance(result, tuple):
        # Try to get column names from view_all_data or fallback
        columns = ["Policy", "Expiry", "Location", "State", "Region", "Investment", "Construction", "BusinessType", "Earthquake", "Flood", "Rating", "id"]
...
def get_chatbot_response(prompt: str, context: Optional[list] = None) -> str:
...
# Add custom CSS for professional color scheme
st.markdown("""
    <style>
        div.stProgress > div > div > div > div {
            background-image: linear-gradient(to right, #444950, #888c94);
        }
        @keyframes gradient {
        ...
        .gradient-text {
            background: linear-gradient(45deg, #888c94, #444950, #23272f);
        ...
        .stMarkdown, .stText, .stMetric, .stSelectbox, .stSlider, .stButton, .stDataFrame {
...
def graphs():
    ...
    investment_by_business_type = df_selection.groupby("BusinessType")["Investment"].count().to_frame()
    ...
    fig_investment = px.bar(
        ...
        color_discrete_sequence=["#888c94"] * len(investment_by_business_type),
        template="plotly_dark",
    )
    fig_investment.update_layout(
        plot_bgcolor="#23272f",
        font=dict(color="#f1f1f1"),
        yaxis=dict(showgrid=True, gridcolor='#444950'),
        xaxis=dict(showgrid=True, gridcolor='#444950'),
    ...
    fig_state = px.line(
        ...
        color_discrete_sequence=["#888c94"],
        template="plotly_dark",
    )
    fig_state.update_layout(
        xaxis=dict(tickmode="linear", gridcolor='#444950'),
        plot_bgcolor="#23272f",
        yaxis=dict(showgrid=True, gridcolor='#444950'),
        font=dict(color="#f1f1f1"),
        margin=dict(t=30, l=10, r=10, b=10)
...
def MLPage():
    ...
    st.info("💡 **Tip**: Use the sidebar filters to select specific data for analysis. More data generally leads to better model performance.")
    ...
    if ml_task == "Investment Prediction":
        ...
        if len(X) < 10:
            st.error("Not enough data for training. Please select more data using the filters.")
            return
        
        if len(feature_cols) == 0:
            st.error("No valid features found for training. Please check your data.")
            return
        ...
        if st.session_state.investment_model is None or st.session_state.feature_cols is None:
        ...
        if len(features) > 0:
            ...
            contamination = float(st.slider("Anomaly Threshold (%)", 1, 10, 5) / 100)
            
            if st.button("Detect Anomalies"):
                with st.spinner("Detecting anomalies..."):
                    try:
                        # Train isolation forest
                        iso_forest = IsolationForest(contamination=float(contamination), random_state=42)  # type: ignore
                        ...
                        fig = px.scatter(
                            ...
                            labels={features[0]: features[0], features[1] if len(features) > 1 else features[0]: features[1] if len(features) > 1 else features[0]},
                            color_discrete_sequence=['#888c94', '#444950']
                        )
                        fig.update_layout(
                            plot_bgcolor="#23272f",
                            font=dict(color="#f1f1f1"),
                ...
                if st.button("Train Renewal Model"):
                    with st.spinner("Training renewal prediction model..."):
                        try:
                            ...
                            st.error(f"Error training renewal model: {str(e)}")
                            st.info("Please check your data and try again.")
            else:
                st.warning("Required features are not available in numeric format.")
        except Exception as e:
...
def sideBar():
    with st.sidebar:
        selected = option_menu(
            ...
            default_index=0,
            styles={
                "container": {"padding": "0!important", "background-color": "#23272f"},
                "icon": {"color": "#888c94", "font-size": "1rem"},
                "nav-link": {
                    "font-size": "0.9rem",
                    "text-align": "left",
                    "margin": "0px",
                ...
                "nav-link-selected": {"background-color": "#888c94"},
            }
        )
